...
# class Conv2dLayer(jit.ScriptModule):
class Conv2dLayer(nn.Module):
    __constants__ = ['emb_dim', 'hidden_dim']

    def __init__(self, config, emb_dim=None, _Cell=None, first_layer=True):
        super().__init__()
        if emb_dim is None:
            self.emb_dim = config.hidden_dim
        else:
        ...
        if first_layer is False and config.cnn_skip is True:
            self.skip = True
        ...
        if config.cnn_model == "hybrid":
            out_dim = self.emb_dim // 2
        else:
            out_dim = self.emb_dim

        assert config.cnn_model in {"plain", "dilation", "hybrid"}
        self.cnn_model = config.cnn_model
        kernel_size = config.cnn_kernel_size

        self.conv_layer1 = torch.nn.Conv2d(in_dim, out_dim,
                                           kernel_size=kernel_size, stride=1,
                                           dilation=1, padding=kernel_size//2*1)
        self.conv_layer2 = torch.nn.Conv2d(in_dim, out_dim,
                                           kernel_size=kernel_size, stride=1,
                                           dilation=2, padding=kernel_size//2*2)

    def forward(self, x: torch.Tensor,
                states: Optional[torch.Tensor],
                masks: torch.Tensor):
        x_in = x
        if states is not None and self.skip is True:
            x = torch.cat([x, states], dim=-1)
        x = x.permute(0, 3, 1, 2)
        if self.cnn_model == "plain":
            out_x = self.conv_layer1(x)
        elif self.cnn_model == "dilation":
            out_x = self.conv_layer2(x)
        elif self.cnn_model == "hybrid":
            out_x1 = self.conv_layer1(x)
            out_x2 = self.conv_layer2(x)
            # print(x.shape)
            # print(out_x1.shape)
            # print(out_x2.shape)
            out_x = torch.cat([out_x1, out_x2], dim=1)
        else:
            raise ValueError(self.cnn_model)

        out_x = out_x.permute(0, 2, 3, 1)
        # masking
        out_x.mul(masks.unsqueeze(-1))
        return out_x, x_in
