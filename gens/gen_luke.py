...
parser.add_argument('--model_name',
                    default='studio-ousia/luke-base',#'albert-xxlarge-v1,studio-ousia/luke-base',
                    action='store',)

parser.add_argument('--save_path',
                    default='../wv/luke.scierc_with_heads.pkl',
                    action='store',)
...
class BertEmbeddingsWithHeads(BertEmbeddings):
    def __init__(
        ...
        elif "luke" in bert_model_or_path:
            self.tokenizer = LukeTokenizer.from_pretrained(bert_model_or_path)
            self.model = LukeModel.from_pretrained(
                pretrained_model_name_or_path=bert_model_or_path,
                output_hidden_states=True,
                output_attentions=True
            )
    ...
    def _add_embeddings_internal(self, sentences: List[Sentence]) -> List[Sentence]:
        ...
        all_encoder_layers = tmp[-3]
        all_attentions = tmp[-2]
            ...
            for sentence_index, sentence in enumerate(sentences):
                ...
                for token in sentence:
                    ...
                    if self.pooling_operation == "first":
                        ...
                        except Exception as e:#即出现空的embedding时，随机进行标准正态初始化
                            print(sentence)
                        ...
                        # if len(embeddings) == 0:#即出现空的embedding时，随机进行标准正态初始化
                        #     # device = torch.device("cuda:0")
                        #     mean = torch.randn(768)#.to(device)
                        #     self.i = self.i+1
                        #     print(self.i, token)
                        # else:
                        #     mean = torch.mean(torch.cat(embeddings, dim=0), dim=0)
...
if args.layers == 'mean':
    ...
    embedding = BertEmbeddingsWithHeads(args.model_name, layers=args.layers, pooling_operation="first")
...
#     s = form_sentence(tokens)
#
#     luke_entities, luke_entity_spans = get_entity_span(item)
#
#     encoding = tokenizer(s, entities=luke_entities, entity_spans=luke_entity_spans, add_prefix_space=True,
#                          return_tensors="pt")
